...
class ViewController: UIViewController,UISearchBarDelegate,DefaultChangeControllerDelegate {
    ...
    func detailController(_ controller: DetailController, didFinishAdding person: Person, name: String, age: Int, tution: Double, startDate: Date , isAdd: Bool) {
        
        
        print("IS WHAT \(isAdd)")
        
        if(isAdd){
            self.appdelegate.insertRecord(name:  name, age: age, tution: tution, startDate: startDate)
        }else{
